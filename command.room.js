...
var commandRoom = {
    ...
    run: function(room) {
        ...
        var buildableExtensionCount = builder.getExtensionCount(room);
       ...
       var buildableTowerCount = builder.getTowerCount(room);
...
function buildModule() { 
    ...
    this.getExtensionCount = function(room){
      var extensions = room.find(FIND_MY_STRUCTURES, {filter: {structureType: STRUCTURE_EXTENSION}}).length;
    ...
    this.getTowerCount = function(room){
      var extensions = room.find(FIND_MY_STRUCTURES, {filter: {structureType: STRUCTURE_TOWER}}).length;
      switch (room.controller.level) {
          case 3:
          case 4:
              return 1;
    ...
    this.buildExtension = function(room){
        ...
        var x;
        var y;
        for (x = spawn.pos.x - 2; x <= spawn.pos.x - 2; x = x + 2) {
            for (y = spawn.pos.y - 2; y <= spawn.pos.y - 2; y = y + 2) {
    ...
    this.buildRoads = function(room){
        ...
        for (var sourceIndex in extensions){
    ...
    this.buildRoadTo = function(room, startPos, targetPos){
        ...
        var foundPath = PathFinder.search(startPos, targetPos);
        /*
        ...
        */
        var currentConstrutions = room.find(FIND_CONSTRUCTION_SITES).length;
        for (var roomPos in foundPath.path){
            var pos = foundPath.path[roomPos];
